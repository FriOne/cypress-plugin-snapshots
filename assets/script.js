...
(function () {
  ...
  function onCompareClick(e) {
    ...
    if (!data || imageUpdated(data)) {
      console.log('Image already updated');
  ...
  function readFile(fullPath, encoding = 'base64', options = { log: false, timeout: 2000 }) {
    ...
    if (!cache[fullPath]) {
      return Cypress.Promise.resolve(cy.readFile(fullPath, encoding, options).then(result => {
  ...
  function formatImageResult(data) {
    ...
    if (data.passed || data.updated) {
      return loadImage(data.expected.path).then((dataUri) =>
    ...
    return loadImage(data.expected.path).then(expectedImage => {
      return loadImage(data.actual.path).then(actualImage => {
        if (!data.diff) {
        ...
        return loadImage(data.diff.path).then(diffImage => {
          return formatPreview(
            title,
            wrapImage('expected', title, expectedImage, imageStyle) +
            wrapImage('actual', title, actualImage, imageStyle) +
  ...
  function loadImage(imagePath) {
    return new Promise((resolve, reject) => {
      const onImageLoad = (base64Image) => {
        socket.off(serverData.actions.LOADED_IMAGE, onImageLoad);

        resolve(`data:image/png;base64,${base64Image}`);
      };

      socket.on(serverData.actions.LOADED_IMAGE, onImageLoad);
      socket.emit(serverData.actions.LOAD_IMAGE, imagePath);
